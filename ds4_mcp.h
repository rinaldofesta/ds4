...
/* Per-server introspection (indexes 0..server_count-1, registration order). */
const char *ds4_mcp_registry_server_name(const ds4_mcp_registry *reg, int i);   /* NULL OOB */
bool        ds4_mcp_registry_server_alive(const ds4_mcp_registry *reg, int i);  /* false OOB */
long        ds4_mcp_registry_server_pid(const ds4_mcp_registry *reg, int i);    /* -1 OOB/never spawned */
int         ds4_mcp_registry_server_tool_count(const ds4_mcp_registry *reg, const char *server_name);

/* Respawn a server by name: if alive, terminate it first (same SIGTERM->grace->
 * SIGKILL group pattern as registry_free, single-server scope); then re-spawn
 * from the RETAINED spawn spec, redo initialize handshake. The existing tool
 * table is KEPT VERBATIM (the system prompt already advertised those wire
 * names mid-session); tools/list is NOT re-fetched -- ds4_agent.c's system
 * prompt is not regenerated by a reconnect. If the restarted server serves a
 * different tool set, a call against a now-stale tool just surfaces as an
 * ordinary "Tool error: ..." (whatever JSON-RPC error the server itself
 * returns for a tools/call naming something it no longer implements), same
 * as any other tool failure. Returns true on success; on failure the server
 * is left dead with a warning in err. */
...
/* The retained spawn spec's argv[0] (command), or NULL OOB/never retained --
 * lets a reconnect test confirm the spec survives multiple respawns
 * untouched (no use-after-free / corruption) without exposing mcp_cfgentry
